...
#if defined(NDEBUG)
# error "profitcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xebcc4ec2917fb9338f9f1d93118672c858f9f7f2e5a04919d5af8632dc1598a5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // profitcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "profitcoin Signed Message:\n";
...
{
    // profitcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // profitcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // profitcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1456742887, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "ABCD";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c34c50b305bb62b631635096622776c6bd6075d629d902808cd1bda365404c2c05ba0dc26b36ded06f266d8b27181f7b714a3e19389b739ab6ea1b3fcbdf2322") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1456742887;
        ...
        assert(block.hashMerkleRoot == uint256("0x7bac3118b6e68e98c20d13083eb7178f7fdbcb340781f18766103af660fce533"));
...
unsigned char pchMessageStart[4] = { 0x42, 0x29, 0x0e, 0xb6 }; // profitcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("profitcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("profitcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static profitcoinMiner(CWallet *pwallet)
{
    printf("profitcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("profitcoin-miner");
    ...
    try { loop {
        ...
        printf("Running profitcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("profitcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&profitcoinMiner, pwallet));
}
